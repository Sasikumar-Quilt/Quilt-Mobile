...
class JournalEditorWidgetState extends BasePageState<JournalEditorWidget> {
  ...
  getArgs() {
    if (!isArg) {
      ...
      if (args["journalObj"] != null) {
        ...
        String documentJson = journalObj!.response!;
        _controller.document = Document.fromJson(jsonDecode(documentJson));
        isEnable = true;
        isEdit = true;
  ...
  Future<void> addOrUpdateJournal() async {
    try {
      ...
      String deltaJsonString = jsonEncode(deltaJson);
      ApiResponse apiResponse;
      if (journalObj != null) {
        apiResponse =
            await apiHelper.updateJournal(journalObj!.id!, deltaJsonString);
      } else {
        apiResponse = await apiHelper.addJournal(
            contentObj!.contentId!, deltaJsonString, date);
      }
      print("apiResponse");
      if (apiResponse.status == Status.ERROR) {
        ...
        Navigator.pushNamed(context, HomeWidgetRoutes.VideoCompletedWidget,
                arguments: {"object": contentObj, "fromJournal": 1})
              ...
              children: [
                Container(
                  ...
                  child: Row(
                    children: [
                      InkWell(
                        child:
                            SvgPicture.asset("assets/images/close_black.svg"),
                        onTap: () {
                          Navigator.of(context).pop({"isMute": isMute});
                        },
                      ),
                      ...
                      isEdit
                          ? GestureDetector(
                              onTapDown: (dertails) {
                                _showPopupMenu(
                                    context, dertails.globalPosition);
                              },
                              child: Container(
               ...
               if(isEnable){
                ...
                Container(
                  ...
                  height: isEdit
                      ? journalObj!.question!.length > 200
                          ? 150
                          : null
                      : contentObj!.contentUrl!.length > 200
                          ? 150
                          : null,
                  child: SingleChildScrollView(
                    child: Text(
                      isEdit ? journalObj!.question! : contentObj!.contentUrl!,
                      textAlign: TextAlign.start,
                      style:
                          TextStyle(fontFamily: "Causten-Medium", fontSize: 18),
                    ),
                  ),
